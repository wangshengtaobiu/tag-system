...
Ontology Factory 的输入来自 `tag_enrichment` 模块（S0 阶段集成在 Factory 内部）：

```
tag_acquisition    →    tag_enrichment (S0)    →    ontology_factory (S1-S8)
[{label, count}]        [{标签名, 分类建议,          冻结本体
                          定义说明, 上位tag,
                          区别, 文学示例词}]
```
...
S0（丰富化）将 tag_acquisition 产出的 `[{label, count}]` 原始数据转化为富格式 `[{标签名, 分类建议, 定义说明, 上位tag, 区别, 文学示例词}]`，供 S1 消费。

S4 和 S5 各自独立运行，不合并。流水线保持 9 阶段完整结构。
